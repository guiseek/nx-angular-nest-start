...
import { UserService } from '@wws/account/shared/data-access';
...
export class UsersContainer implements OnInit {
  ...
  tableConfig: TableConfig = {
    columns: [
      { columnDef: 'id', header: '#', cell: element => element.id },
      {
        columnDef: 'name',
        header: 'Nome',
        cell: element => element.name.first
      },
      { columnDef: 'email', header: 'Email', cell: element => element.email },
      {
        columnDef: 'isActive',
        header: 'Status',
        cell: element => element.isActive
      }
    ],
  ...
  onCreateUser(data: IUser) {
    console.log(data);
    this.userService.createOne(data).subscribe(res => {
      console.log(res);
