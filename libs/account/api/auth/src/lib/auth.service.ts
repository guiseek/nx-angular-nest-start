...
export class AuthService {
  ...
  async login({
    ...
    email,
    name
  }: Partial<User>): Promise<AuthSuccessResponse> {
  ...
  async forgotPassword(dto: ForgotPasswordDto) {
    try {
      const { resetPassword, ...data } = await this.usersService.forgotPassword(
        dto
      );
      if (resetPassword) {
        await this.mailerService.forgotPassword(resetPassword.token, dto.email);
        return {
          id: data.id,
          resetPassword
        };
